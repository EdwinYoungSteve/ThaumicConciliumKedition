...
import com.keletu.thaumicconcilium.client.gui.InventoryThaumaturge;
    ...
    private void generateContents() {
        if (!theWorld.isRemote && !inventory.getStackInSlot(0).isEmpty() && inventory.getStackInSlot(1).isEmpty() && inventory.getStackInSlot(2).isEmpty() && inventory.getStackInSlot(3).isEmpty() && inventory.getStackInSlot(4).isEmpty() && thaumaturge.isValued(inventory.getStackInSlot(0))) {
            ...
            if (offer.getItem() == ItemsTC.curio) {
                ...
                switch (choise) {
                    case 0:
                        research = new ItemStack(TCItems.research_page, 1, 5);
                        this.mergeItemStack(research, 1, 5, false);
                        this.inventory.decrStackSize(0, 1);
                        break;
                    case 1:
                    case 2:
                        research = new ItemStack(TCItems.research_page, 1, 1);
                        this.mergeItemStack(research, 1, 5, false);
                        this.inventory.decrStackSize(0, 1);
                        break;
                    case 3:
                        research = new ItemStack(TCItems.research_page, 1, 2);
                        this.mergeItemStack(research, 1, 5, false);
                        this.inventory.decrStackSize(0, 1);
                        break;
                    default:
