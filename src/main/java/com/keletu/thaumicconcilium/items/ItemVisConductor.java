                    ...
                    if (ThaumcraftCapabilities.knowsResearch(player, "DEMATUPGRADE") && wandstack.getItemDamage() == 2) {
                        ...
                        if (look instanceof EntityLivingBase) {
                            if (!world.isRemote) {
                                ...
                                ThaumicConcilium.packetInstance.sendToAllAround(new PacketFXLightning((float) look.posX, (float) look.posY + 0.5f, (float) look.posZ, (float) player.posX, (float) player.posY - 0.5f, (float) player.posZ, rgb, 0.2F), new NetworkRegistry.TargetPoint(world.provider.getDimension(), player.posX, player.posY, player.posZ, 32.0));
                            }
                        }
