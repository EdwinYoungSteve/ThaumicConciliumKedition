...
import com.keletu.thaumicconcilium.entity.EntityHexRift;
                ...
                if (hasRift) {
                    ...
                    if (essentia.size() != 0 && !near.isEmpty()) {
                        for (EntityItem e : near) {
                            ...
                            if (es != null && es.getItem() == TCItems.void_slag) {
                                heat += 2;
                                this.markDirty();
                                this.world.notifyBlockUpdate(this.pos, world.getBlockState(pos), world.getBlockState(pos), 3);
                            }
                            ...
                            if (heat > 0) {
                                if (world.getTotalWorldTime() % 60 == 0 && world.rand.nextInt(10) >= 8) {
                                    ...
                                    for (int i = 0; i < rand; i++) {
                                        EntityItem slag = new EntityItem(this.world, pos.getX() + (-0.5 + world.rand.nextDouble()) * 2.0, pos.getY() + 2, pos.getZ() + (-0.5 + world.rand.nextDouble()) * 2.0, new ItemStack(TCItems.void_slag, 1));
