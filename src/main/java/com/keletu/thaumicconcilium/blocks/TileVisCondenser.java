...
public class TileVisCondenser extends TileThaumcraft implements ITickable {
    ...
    public void shoot(TileDestabilizedCrystal crystal, BlockPos pos, int power) {
        ...
        if (!this.world.isRemote) {
            ...
            ThaumicConcilium.packetInstance.sendToAllAround(new PacketFXLightning(this.getPos().getX() + 0.5F, this.getPos().getY() + 1.5F, this.getPos().getZ() + 0.5F, x + 0.5F, y + 0.5F, z + 0.5F, c, 0.01F), new NetworkRegistry.TargetPoint(world.provider.getDimension(), x, y, z, 32.0));
        }
    }
