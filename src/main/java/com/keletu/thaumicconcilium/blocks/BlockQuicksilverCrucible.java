    ...
    public void randomDisplayTick(IBlockState stateIn, World w, BlockPos pos, Random r) {
        if (r.nextInt(10) == 0) {
            ...
            if (te instanceof TileQuicksilverCrucible && ((TileQuicksilverCrucible) te).aspects.getAmount(Aspect.EXCHANGE) > 0) {
                w.playSound(pos.getX(), pos.getY(), pos.getZ(), new SoundEvent(new ResourceLocation(ThaumicConcilium.MODID + ":melted")), SoundCategory.BLOCKS, 1.1F + r.nextFloat() * 0.1F, 1.2F + r.nextFloat() * 0.2F, false);
            }
        }
