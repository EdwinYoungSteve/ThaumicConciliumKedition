...
public class EventHandlerEntity {
    ...
    public static void onEntityJoin(EntityJoinWorldEvent event) {
        if (event.getEntity() instanceof EntityBrainyZombie || event.getEntity() instanceof EntityGiantBrainyZombie) {
            if (event.getWorld().rand.nextInt(100) > ConfigsTCo.madThaumaturgeReplacesBrainyZombieChance) {
        ...
        if (event.getEntity() instanceof EntityCultistKnight) {
            if (event.getWorld().rand.nextInt(100) > ConfigsTCo.crimsonPaladinReplacesCultistWarriorChance) {
                if (!event.getWorld().isRemote) {
    ...
    public static void attachCapabilitiesPlayer(AttachCapabilitiesEvent<Entity> event) {
        if (event.getObject() instanceof EntityPlayer) {
            event.addCapability(
                    new ResourceLocation(ThaumicConcilium.MODID, "thaumic_concilium"),
