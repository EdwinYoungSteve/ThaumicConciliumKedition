...
public class EntityCompressedBlast extends EntityThrowable implements IEntityAdditionalSpawnData {
    ...
    public void onUpdate() {
        ...
        if (this.world.isRemote && this.fireworkAge % 5 == 0) {
            ThaumicConcilium.proxy.smeltFX(this.posX, this.posY, this.posZ, this.world, 1);
        }

        if (this.fireworkAge % 5 == 0) {
            this.world.playSound(null, this.getPosition(), new SoundEvent(new ResourceLocation(ThaumicConcilium.MODID + ":thump")), SoundCategory.PLAYERS, 0.9F, 2.6F + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.8F);
        }
