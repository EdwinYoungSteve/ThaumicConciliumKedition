...
import com.keletu.thaumicconcilium.packet.PacketEnslave;
...
public class EntityCrimsonPontifex extends EntityThaumcraftBoss implements IRangedAttackMob {
    ...
    protected SoundEvent getAmbientSound() {
        return new SoundEvent(new ResourceLocation(ThaumicConcilium.MODID + ":chant"));
    }

    @Override
    protected SoundEvent getDeathSound() {
        return new SoundEvent(new ResourceLocation(ThaumicConcilium.MODID + ":growl"));
    }
    ...
    protected void setEquipmentBasedOnDifficulty(DifficultyInstance diff) {
        this.setHeldItem(EnumHand.MAIN_HAND, new ItemStack(TCItems.molot));
                ...
                if (continuousAttack && attackCounter < 10) {
                    ...
                    if (!world.isRemote) {
                        if (targetedEntity instanceof EntityPlayer) {
                            ThaumicConcilium.packetInstance.sendTo(new PacketEnslave(targetedEntity.getName(), false), (EntityPlayerMP) targetedEntity);
                        }
                    }
                            ...
                            switch (rand) {
                                ...
                                case 1: {
                                    if (targetedEntity instanceof EntityPlayer) {
                                        ...
                                        ThaumicConcilium.packetInstance.sendTo(new PacketEnslave(name, true), (EntityPlayerMP) targetedEntity);
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase target, float distanceFactor) {
        if (this.canEntityBeSeen(target)) {
            ...
            shackles.playSound(new SoundEvent(new ResourceLocation(ThaumicConcilium.MODID + ":shackles_throw")), 0.5F, 1.0F);
