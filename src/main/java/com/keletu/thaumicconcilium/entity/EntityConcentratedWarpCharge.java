...
public class EntityConcentratedWarpCharge extends Entity implements IEntityOwnable, IEntityAdditionalSpawnData {
    ...
    public void onUpdate() {
        ...
        if (!byForce && (ticksExisted % 120 == 0 || ticksExisted == 0)) {
            world.playSound(null, this.getPosition(), new SoundEvent(new ResourceLocation(ThaumicConcilium.MODID + ":whispers")), SoundCategory.HOSTILE, 0.1F, 1.0F);
        }
        ...
        if (this.world.isRemote) {
            if (getOwner() != null) {
                ...
                if (this.massHysteria && this.world.rand.nextInt() % 8 == 0) {
                    for (int i = 0; i < 10; i++) {
                        ThaumicConcilium.proxy.taintsplosion(this.world, this.posX, this.posY, this.posZ);
                    }
                }
            }
        }
