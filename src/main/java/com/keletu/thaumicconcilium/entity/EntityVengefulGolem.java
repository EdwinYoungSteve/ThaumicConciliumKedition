...
public class EntityVengefulGolem extends EntityMob implements IRangedAttackMob {
    ...
    protected SoundEvent getAmbientSound() {
        return new SoundEvent(new ResourceLocation(ThaumicConcilium.MODID + ":gnaw"));
    }
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase target, float f) {
        if (target instanceof EntityPlayer) {
            ...
            if (!muffed && rand.nextInt(10) >= 2) {
                ...
                ThaumicConcilium.packetInstance.sendTo(new PacketEnslave(name, true), (EntityPlayerMP) target);
            } else {
                ThaumicConcilium.packetInstance.sendTo(new PacketEnslave(name, false), (EntityPlayerMP) target);
            }
